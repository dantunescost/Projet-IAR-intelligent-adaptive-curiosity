...
# variables pour la création du 1er expert
...
# Creation du premier expert (unique), child1 contient l'indice de l'enfant dans DATA contenant les valeurs en-dessous de cutVal
...
def bouclePrincipale():
    ...
    while t < NB_ITERATIONS:
        ...
        for i in range(len(possibleActions)):
            ...
            tempLE = list(expert['LE']) #on clone LE de l'expert concerné
...
def cutExpert(expert,taille):           #Fonction qui permet de générer 2 experts à partir d'un expert parent
    ...
    indexCutVal = -1
    cutVal = -99
    for i in range(3):                  #on parcoure les dimensions speedLeft, speedRight, Frequence
...
def expertAUtiliser(action):    #Cette fonction permet de trouver le meilleur expert pour l'action en parcourant le pseudo-arbre
    expert = DATA[0]
