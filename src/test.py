...
data_MP = []    #de la forme [speedLeft,speedRight,frequence,distance avant action,erreur de prediction]
...
def bouclePrincipale():
    ...
    while t < NB_ITERATIONS:
        ...
        for i in range(len(possibleActions)):
            ...
            Ep = MetaPredictionMP(possibleActions[i],distanceInitiale) #calcul de la prediction de l'erreur
...
def MetaPredictionMP(action,distanceInitiale):
    ...
    if len(data_MP) < K:
        ...
        for i in range(len(data_MP)):
            ...
            d4=abs(data_MP[i][3]-distanceInitiale)
...
def cutExpert(expert,taille):       #Fonction qui permet de générer 2 experts à partir d'un expert parent
    ...
    indexCutVal = -1
    cutVal = -99
    for i in range(len(expert['data_P'][0])-2):     #on parcoure les dimensions speedLeft, speedRight, Frequence
        ...
        for j in range((len(dataCopy)//2)-(len(dataCopy)//20),(len(dataCopy)//2)+(len(dataCopy)//20)):  #on cherche à minimiser la somme des variances de chaque sous-liste
            var = np.std(dataCopy[:j][len(expert['data_P'][0])-1]) + np.std(dataCopy[j:][len(expert['data_P'][0])-1])
            if var < stdMin:
                stdMin = var
                dim = i
                indexCutVal = j
